 ...
 ******************************************************************************
 * @file    main.c
 * @brief   STSAFE-A120 Symmetric key wrapped provisioning — AES-128 CMAC
 ******************************************************************************
 *           COPYRIGHT 2022 STMicroelectronics
 ******************************************************************************
 * Usage:  ./05_Symmetric_key_provisioning_wrapped_compute_AES-128_CMAC [busID]
 *
 * Provisions an AES-128-CMAC key by wrapping with an ECDH-derived KEK.
...
/* Static key to be wrapped and provisioned */
const uint8_t aes_128_cmac_key[AES_128_CMAC_KEY_SIZE] = {
...
int main(int argc, char *argv[]) {
    ...
    printf("--------------------------------------------------------------------------------------------------------------------");
    printf("\n\r-       STSAFE-A120 Symmetric key wrapped provisioning + AES-128-CMAC example                                    -");
    printf("\n\r--------------------------------------------------------------------------------------------------------------------");
    ...
    stsafea_close_host_session(&host_session_handler);
    printf(PRINT_GREEN "\n\n\r - Wrapped AES-128-CMAC provision+compute+verify : SUCCESS\n\r" PRINT_RESET);
    return 0;
}
